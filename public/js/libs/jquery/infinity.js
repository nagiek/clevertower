...
(function (factory) {
  ...
  // Config:
  config.PAGE_TO_SCREEN_RATIO = 0.85;
  ...
  function ListView($el, options) {
    ...
    this.addFn = options.add || null;
    this.stashFn = options.stash || null;
  ...
  ListView.prototype.prepend = function(obj) {
    ...
    // Index starts at 1 to skip the first page.
    var index = 1, length, firstPage, curr
        item = convertToItem(this, obj),
        pages = this.pages;
    ...
    // Recalculate the coords of subsequent pages.
    for(index; index < length; index++) {
      curr = pages[index];
      curr.top += item.height;
      curr.bottom += item.height;
    }
  ...
  function insertPagesInView(listView) {
    ...
    for(index; index < length; index++) {
      ...
      if(!inOrder) {
        curr.stash(listView.$shadow, listView.stashFn);
        curr.appendTo(listView.$el, listView.addFn);
  ...
  Page.prototype.appendTo = function($el) {
    ...
    if(!this.onscreen) {
      this.$el.appendTo($el);
      this.onscreen = true;
      // Only call this if we have already loaded.
      if (this.parent.addFn && this.stashed) {
        for (index = 0, length = this.$el.length; index < length; index++) {
          this.parent.addFn.call(this.$el[index], this.$el[index]);
        }
      }
    }
  };
  ...
  Page.prototype.stash = function($el) {
    ...
    if(this.onscreen) {
      ...
      this.onscreen = false;
      this.stashed = true;
      if (this.parent.stashFn) {
        for (index = 0, length = this.$el.length; index < length; index++) {
          this.parent.stashFn.call(this.$el[index], this.$el[index]);
        }
      }
    }
  };
  ...
  Page.prototype.cleanup = function() {
    ...
    if (this.parent.stashFn) this.parent.stashFn.call(this.$el, this.$el)
    this.parent = null;
