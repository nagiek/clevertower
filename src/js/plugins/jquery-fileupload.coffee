...
((factory) ->
  ...
  $.propHooks.elements = get: (form) ->
    if $.nodeName(form, "form")
      return $.grep(form.elements, (elem) ->
        not $.nodeName(elem, "input") or elem.type isnt "file"
      )
    null
  ...
  $.widget "blueimp.fileupload",
    ...
    _onProgress: (e, data) ->
      if e.lengthComputable
        ...
        data.loaded = loaded
        data.total = total
        data.bitrate = data._bitrateTimer(now, loaded, data.bitrateInterval)
        ...
        @_trigger "progressall", e,
          ...
          loaded: @_loaded
          total: @_total
          bitrate: @_bitrateTimer(now, @_loaded, data.bitrateInterval)
        ...
        if options.postMessage
          ...
          if options.blob
            ...
            $.each options.files, (index, file) ->
              ...
              # dummy objects:
              formData.append options.paramName[index] or paramName, file, file.name  if (window.Blob and file instanceof Blob) or (window.File and file instanceof File)
    ...
    _initFormSettings: (options) ->
      ...
      options.type = "POST"  if options.type isnt "POST" and options.type isnt "PUT" and options.type isnt "PATCH"
    ...
    _chunkedUpload: (options, testOnly) ->
      ...
      upload = ->
        ...
        jqXHR = ((that._trigger("chunksend", null, o) isnt false and $.ajax(o)) or that._getXHRPromise(false, o.context)).done((result, textStatus, jqXHR) ->
          ub = that._getUploadedBytes(jqXHR) or (ub + o.chunkSize)
          if not o.loaded or o.loaded < o.total
    ...
    _onDone: (result, textStatus, jqXHR, options) ->
      if not @_isXHRUpload(options) or not options.loaded or options.loaded < options.total
        total = @_getTotal(options.files) or 1
    ...
    _onDrop: (e) ->
      ...
      data = {}
      e.preventDefault()  if dataTransfer and dataTransfer.files and dataTransfer.files.length
    ...
    _onDragOver: (e) ->
      ...
      if dataTransfer and $.inArray("Files", dataTransfer.types) isnt -1
